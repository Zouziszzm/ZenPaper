...
import { Download, LayoutGrid, Palette, Type, ScanLine, Smartphone, Monitor, ChevronDown, Check, MousePointer2, Minus, Grid, Eraser, Moon, Sun, LayoutTemplate, Circle, Square } from "lucide-react";
    ...
    return (
        ...
        >
            ...
            <div className="flex items-center gap-1 border-r border-zinc-200 dark:border-zinc-800 pr-3 pl-1">
                <button
                    onClick={() => store.setDrawMode('select')}
                    className={cn("p-2 rounded-xl transition-all", store.drawMode === 'select' ? "bg-zinc-200 dark:bg-zinc-800 text-blue-600 dark:text-blue-400" : "hover:bg-zinc-100 dark:hover:bg-zinc-800")}
                    title="Select Tool"
                >
                ...
                <button
                    onClick={() => store.setDrawMode('horizontal-line')}
                    className={cn("p-2 rounded-xl transition-all", store.drawMode === 'horizontal-line' ? "bg-zinc-200 dark:bg-zinc-800 text-blue-600 dark:text-blue-400" : "hover:bg-zinc-100 dark:hover:bg-zinc-800")}
                ...
                <button
                    onClick={() => store.setDrawMode('vertical-line')}
                    className={cn("p-2 rounded-xl transition-all", store.drawMode === 'vertical-line' ? "bg-zinc-200 dark:bg-zinc-800 text-blue-600 dark:text-blue-400" : "hover:bg-zinc-100 dark:hover:bg-zinc-800")}
                    title="Vertical Line Tool"
                >
                    <Minus className="w-5 h-5 rotate-90" />
                </button>

                <button
                    onClick={() => store.setDrawMode('eraser')}
                    className={cn("p-2 rounded-xl transition-all", store.drawMode === 'eraser' ? "bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400" : "hover:bg-zinc-100 dark:hover:bg-zinc-800")}
                    title="Eraser Tool"
                >
                    <Eraser className="w-5 h-5" />
                </button>

                <div className="w-8 h-8 rounded-full border-2 border-zinc-200 dark:border-zinc-700 ml-2 relative overflow-hidden transition-transform hover:scale-110" title="Line Color">
                    <div className="absolute inset-0 pointer-events-none rounded-full" style={{ backgroundColor: store.lineColor }}></div>
                    <input
                        type="color"
                        value={store.lineColor}
